...
import com.zitian.csims.ui.widget.CustomDialogStyle4;
...
public class InventoryInputDisperseErrorActivity extends BaseActivity implements  View.OnClickListener, HttpListener<String> {
    ...
    InventoryInputDisperse.Bean itemModel;
    List<InventoryInputDisperseError.Bean> list = new ArrayList<>();

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inventory_input_disperse_error);
        mCurrentPage = 0;
        initView();
        setViewListener();
        //DataBind();
    ...
    private void initView() {
        ...
        itemModel = (InventoryInputDisperse.Bean) bundle.get("itemModel");
        prodNo.setText("库位号："+itemModel.getWh_wareHouseNo());

        mRecyclerView = (LRecyclerView) findViewById(R.id.list);
        mDataAdapter = new InventoryInputDisperseErrorAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    InventoryInputDisperseError.Bean item = mDataAdapter.getDataList().get(position);
    ...
    private void DataBind2(boolean isLoading) {
        Gson gson = new Gson();
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.InventoryInputDisperseError(), RequestMethod.POST);
    ...
    private void DataBind3(boolean isLoading) {
        Gson gson = new Gson();
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.GetInventoryInputDisperseError(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("sEcho", "1");
        SENDSMSRequest.add("UserName", CSIMSApplication.getAppContext().getUser().getO_name());
        SENDSMSRequest.add("UserId", CSIMSApplication.getAppContext().getUser().getO_id());
    ...
    private InventoryInputDisperseErrorAdapter mDataAdapter = null;
    ...
    private void addItems(List<InventoryInputDisperseError.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            ...
            case 1001://快速登录
                ...
                if (warehousingInDistributionQualityInput.getResult() == 1) {
                    Toast.makeText(InventoryInputDisperseErrorActivity.this,warehousingInDistributionQualityInput.getMsg(),Toast.LENGTH_SHORT).show();
                    String result = itemModel.getWh_wareHouseNo().toString().trim();
                ...
                break;
            case 1002://快速登录
                InventoryInputDisperseError inventoryInputDisperseError = gson.fromJson(response.get(), InventoryInputDisperseError.class);
                if (inventoryInputDisperseError.getResult() == 1) {
                    //List<InventoryInputWholeError.Bean> list = new ArrayList<>();
                    list = inventoryInputDisperseError.getData();
                    DataBind();
                }else
                {
                    CreateCustomDialogStyle();
     ...
     * item＋item里的控件点击监听事件
     */
    private InventoryInputDisperseErrorAdapter.OnItemClickListener MyItemClickListener = new InventoryInputDisperseErrorAdapter.OnItemClickListener() {
        @Override
        public void onItemClick(View v, InventoryInputDisperseErrorAdapter.ViewName viewName, int position) {
            ...
            switch (v.getId()) {
                case R.id.btnGain:
                    ...
                    InventoryInputDisperseError.Bean  itemModel = mDataAdapter.getDataList().get(position);
                    //itemModel.id = 100;
    ...
    {
        builder = new CustomDialogStyle3.Builder(InventoryInputDisperseErrorActivity.this);
        builder.setTitle("库位号:"+itemModel.getWh_wareHouseNo())
                ...
                .setPositionButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        {
                            Toast.makeText(InventoryInputDisperseErrorActivity.this,"产品编码不能为空！",Toast.LENGTH_SHORT).show();
                            return;
                        }

                        InventoryInputDisperseError.Bean bean = new InventoryInputDisperseError.Bean();
    ...
    public void UpdateCustomDialogStyle(InventoryInputDisperseError.Bean bean,int pos)
    {
        builder = new CustomDialogStyle3.Builder(InventoryInputDisperseErrorActivity.this);
        builder.setTitle("库位号:"+itemModel.getWh_wareHouseNo())
                //.setMessage("请输入产品编码")
...
//    CustomDialogStyle4.Builder builder = null;
//    public void CreateCustomDialogStyle()
//    {
//        builder = new CustomDialogStyle4.Builder(InventoryInputDisperseErrorActivity.this);
//        builder.setTitle("库位号:"+itemModel.getWh_wareHouseNo())
//                .setProdNo(list.size()!=0 ? "" : itemModel.getWh_prodNo())
//                .setProdName(list.size()!=0 ? "" : itemModel.getWh_prodName())
//
