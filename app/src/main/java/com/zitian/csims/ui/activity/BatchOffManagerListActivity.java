...
public class BatchOffManagerListActivity   extends BaseActivity implements  View.OnClickListener, HttpListener<String> {

    private ImageView icon_back;
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_batch_off_manager_list);
    ...
    private void initView() {
        ...
        mDataAdapter = new BatchOffManagerListAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    BatchOffManagerList.Bean item = mDataAdapter.getDataList().get(position);
    ...
    private BatchOffManagerListAdapter mDataAdapter = null;
    ...
    {
        String condition =  "[t_taskType] = '质量问题下架'  ";
        condition += " and t_taskState =  '已领取'";

        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.BatchOffManagerList(), RequestMethod.POST);
    ...
    private void DataBind2(int id,String fromNo,boolean isLoading)
    {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.BatchOffManagerList2(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("userName", CSIMSApplication.getAppContext().getUser().getO_id());
        SENDSMSRequest.add("t_ID", id);
        SENDSMSRequest.add("wh_wareHouseNo", fromNo);
        SENDSMSRequest.add("userName", CSIMSApplication.getAppContext().getUser().getO_id());
        CallServer.getInstance().add(this, 1000, SENDSMSRequest, this, false, isLoading);
        mRecyclerView.setPullRefreshEnabled(true);
        mCurrentPage++;
    ...
    private void addItems(List<BatchOffManagerList.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case 1000://快速登录
                BatchOffManagerList outofSpaceForkliftList = gson.fromJson(response.get(), BatchOffManagerList.class);
     ...
     * item＋item里的控件点击监听事件
     */
    private BatchOffManagerListAdapter.OnItemClickListener MyItemClickListener = new BatchOffManagerListAdapter.OnItemClickListener() {

        @Override
        public void onItemClick(View v, BatchOffManagerListAdapter.ViewName viewName, int position) {
...
//                    sPcount = getIntent().getIntExtra("sPcount",0);;//移入单 总数量
                    ...
                    BatchOffManagerList.Bean itemModel = mDataAdapter.getDataList().get(position);
                    Bundle mBundle = new Bundle();
                    mBundle.putInt("mType",0);
                    //mBundle.putString("sexception",itemModel.getT_exception()); //爆仓区（库位表）主键ID
                    //mBundle.putInt("t_ID",itemModel.getT_ID());//任务表Id
                    ...
                    item.setE_input_prodNo(""); //"e_input_prodNo": "",
                    item.setE_input_prodName(""); //"e_input_prodName": "",
                    item.setE_input_wareHouseNo(""); //"e_input_wareHouseNo": "产品纠错区",
                    item.setE_input_nbox_num(0); // "e_input_nbox_num": 0,
                ...
                case R.id.tv_toNoAndArea:
                    ...
                    item.setE_input_wareHouseNo(itemModel.getT_fromNo()); //"e_input_wareHouseNo": "产品纠错区",
