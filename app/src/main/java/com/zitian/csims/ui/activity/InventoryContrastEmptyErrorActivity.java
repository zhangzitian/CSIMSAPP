...
import com.zitian.csims.model.InventoryEmptyError;
import com.zitian.csims.model.InventoryInputWhole;
import com.zitian.csims.model.InventoryInputWholeError;
...
import com.zitian.csims.ui.adapter.InventoryEmptyErrorAdapter;
import com.zitian.csims.ui.adapter.InventoryInputDisperseErrorAdapter;
import com.zitian.csims.ui.adapter.InventoryInputWholeErrorAdapter;
...
public class InventoryContrastEmptyErrorActivity  extends BaseActivity implements  View.OnClickListener, HttpListener<String> {
    ...
    InventoryContrastList.Bean itemModel;
    //List<InventoryEmptyError.Bean> list = new ArrayList<>();
    //List<InventoryInputWholeError.Bean> list = new ArrayList<>();
    //InventoryContrastList.Bean itemModel;
    List<InventoryEmptyError.Bean> list = new ArrayList<>();
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inventory_empty_error);
        mCurrentPage = 0;
        list.clear();
        initView();
        setViewListener();
    ...
    private void initView() {
        ...
        prodNo.setText("库位号："+itemModel.getWh_wareHouseNo());

        mRecyclerView = (LRecyclerView) findViewById(R.id.list);
        mDataAdapter = new InventoryEmptyErrorAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    InventoryEmptyError.Bean item = mDataAdapter.getDataList().get(position);
        ...
        Gson gson = new Gson();
        if(itemModel.getR_errorJson()!="")
            list = gson.fromJson(itemModel.getR_errorJson(), new TypeToken<List<InventoryEmptyError.Bean>>(){}.getType());
    ...
    private void DataBind2(boolean isLoading) {
        Gson gson = new Gson();
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.InventoryContrastEmptyError(), RequestMethod.POST);
        ...
        SENDSMSRequest.add("errorJson",  gson.toJson(list) );
...
//        Gson gson = new Gson();
//        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.GetContrastInventoryDataAndEmptyError(), RequestMethod.POST);
//        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
//        SENDSMSRequest.add("sEcho", "1");
//        SENDSMSRequest.add("UserName", CSIMSApplication.getAppContext().getUser().getO_name());
//        SENDSMSRequest.add("UserId", CSIMSApplication.getAppContext().getUser().getO_id());
...
//    }
    ...
    private InventoryEmptyErrorAdapter mDataAdapter = null;
    ...
    private void addItems(List<InventoryEmptyError.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            ...
            case 1001://快速登录
                ...
                break;
//            case 1002://快速登录
//                InventoryEmptyError inventoryEmptyError = gson.fromJson(response.get(), InventoryEmptyError.class);
                //if (inventoryEmptyError.getResult() == 1) {
                    //List<InventoryInputWholeError.Bean> list = new ArrayList<>();
                    //list = inventoryEmptyError.getData();
                    //DataBind();
                //}else
                //{
                    //CreateCustomDialogStyle();
                    //ToastUtil.show(this, inventoryInputDisperseError.getMsg());
                //}
//                if (inventoryEmptyError.getResult() == 1) {
//                    itemModel = inventoryEmptyError.getData();
//                    if(CSIMSApplication.getAppContext().getUser().getO_id().equals(inventoryEmptyError.getData().getU1_userid()) && inventoryEmptyError.getData().getU1_errorJson().length()>0)
//                    {
//                        list = gson.fromJson(inventoryEmptyError.getData().getU1_errorJson(), new TypeToken<List<InventoryEmptyError.Bean>>(){}.getType());
//                        DataBind();
//                    }else if(CSIMSApplication.getAppContext().getUser().getO_id().equals(inventoryEmptyError.getData().getU2_userid()) && inventoryEmptyError.getData().getU2_errorJson().length()>0)
//                    {
//                        list = gson.fromJson(inventoryEmptyError.getData().getU2_errorJson(), new TypeToken<List<InventoryEmptyError.Bean>>(){}.getType());
//                        DataBind();
//                    }else
//                    {
//                        CreateCustomDialogStyle();
...
//                break;
     ...
     * item＋item里的控件点击监听事件
     */
    private InventoryEmptyErrorAdapter.OnItemClickListener MyItemClickListener = new InventoryEmptyErrorAdapter.OnItemClickListener() {
        @Override
        public void onItemClick(View v, InventoryEmptyErrorAdapter.ViewName viewName, int position) {
            ...
            switch (v.getId()) {
                case R.id.btnGain:
                    ...
                    InventoryEmptyError.Bean  itemModel = mDataAdapter.getDataList().get(position);
                    //itemModel.id = 100;
    ...
    public void CreateCustomDialogStyle()
    {
        ...
        builder = new CustomDialogStyle3.Builder(InventoryContrastEmptyErrorActivity.this);
        builder.setTitle("库位号:"+itemModel.getWh_wareHouseNo())
                .setProdNo(list.size()!=0 ? "" : itemModel.getWh_prodNo())
                .setProdName(list.size()!=0 ? "" : itemModel.getWh_prodName())

                .setNbox_num("")//.setNbox_num(list.size()!=0 ? "" : String.valueOf(itemModel.getWh_nbox_num()))
                .setNpack_num("")//.setNpack_num(list.size()!=0 ? "" : String.valueOf(itemModel.getWh_npack_num()))
                .setNbook_num("")//.setNbook_num(list.size()!=0 ? "" : String.valueOf(itemModel.getWh_nbook_num()))
                ...
                .setPositionButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        {
                            Toast.makeText(InventoryContrastEmptyErrorActivity.this,"产品编码不能为空！",Toast.LENGTH_SHORT).show();
                            return;
                        }

                        InventoryEmptyError.Bean bean = new InventoryEmptyError.Bean();
    ...
    public void UpdateCustomDialogStyle(InventoryEmptyError.Bean bean,int pos)
    {
        builder = new CustomDialogStyle3.Builder(InventoryContrastEmptyErrorActivity.this);
        builder.setTitle("库位号:"+itemModel.getWh_wareHouseNo())
                //.setMessage("请输入产品编码")
