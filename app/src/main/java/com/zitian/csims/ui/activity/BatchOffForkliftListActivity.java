...
public class BatchOffForkliftListActivity  extends BaseActivity implements  View.OnClickListener, HttpListener<String> {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_batch_off_forklift_list);
        mCurrentPage = 0;
        initView();
        setViewListener();
        DataBind(true);
    ...
    private void initView() {
        ...
        mDataAdapter = new BatchOffForkliftListAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    BatchOffForkliftList.Bean item = mDataAdapter.getDataList().get(position);
     ...
     * 服务器端一共多少条数据
     */
    private static int TOTAL_COUNTER = 100000;//如果服务器没有返回总数据或者总页数，这里设置为最大值比如10000，什么时候没有数据了根据接口返回判断
    /**
     * 每一页展示多少条数据
     */
    ...
    private BatchOffForkliftListAdapter mDataAdapter = null;
    ...
    private WeakHandler mHandler = new WeakHandler() {
        ...
        private void DataBind(boolean isLoading) {
            ...
            //condition += " and t_taskState !=  '已完成'";
            condition += " and (t_taskState =  '未领取' or t_taskState =  '已领取')";
            Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.BatchOffForkliftList(), RequestMethod.POST);
    ...
    private void DataBind2(int id,boolean isLoading) {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.BatchOffForkliftList2(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        ...
        private void addItems(List<BatchOffForkliftList.Bean> list) {
        ...
        public void onSucceed(int what, Response<String> response) {
            ...
            switch (what) {
                case 1000://快速登录
                    BatchOffForkliftList outofSpaceForkliftList = gson.fromJson(response.get(), BatchOffForkliftList.class);
                    if (outofSpaceForkliftList.getResult() == 1) {
                        if (outofSpaceForkliftList.getData() != null) {
                            ...
                            Toast.makeText(BatchOffForkliftListActivity.this,outofSpaceForkliftList.getMsg(),Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
                case 1001://快速登录
                    outofSpaceForkliftList = gson.fromJson(response.get(), BatchOffForkliftList.class);
                    if (outofSpaceForkliftList.getResult() == 1) {
        ...
        private BatchOffForkliftList.Bean itemModel = null;
        private int pos = 0;
        /**
         * item＋item里的控件点击监听事件
         */
        private BatchOffForkliftListAdapter.OnItemClickListener MyItemClickListener = new BatchOffForkliftListAdapter.OnItemClickListener() {

            @Override
            public void onItemClick(View v, BatchOffForkliftListAdapter.ViewName viewName, int position) {
                ...
                switch (v.getId()) {
                    case R.id.tv_fromNoAndArea:
                        ...
                        mBundle.putString("sourceType","质量下架纠错");
