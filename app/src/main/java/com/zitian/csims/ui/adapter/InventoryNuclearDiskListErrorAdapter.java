...
public class InventoryNuclearDiskListErrorAdapter extends ListBaseAdapter<InventoryNuclearDiskListError.Bean>  implements View.OnClickListener{

    public InventoryNuclearDiskListErrorAdapter(Context context) {
        super(context);
    }

    @Override
    public int getLayoutId() {
        return R.layout.activity_inventory_input_whole_error_item;
    }

    @Override
    public void onBindItemHolder(SuperViewHolder holder, int position) {
        InventoryNuclearDiskListError.Bean item = mDataList.get(position);
        ...
        //title3.setOnClickListener(InventoryInputWholeErrorAdapter.this);
        ...
        btnGain.setOnClickListener(InventoryNuclearDiskListErrorAdapter.this);
        btnOperation.setOnClickListener(InventoryNuclearDiskListErrorAdapter.this);
...
//        }
    ...
    public interface OnItemClickListener  {
        void onItemClick(View v, InventoryNuclearDiskListErrorAdapter.ViewName viewName, int position);
        void onItemLongClick(View v);
    }
    private InventoryNuclearDiskListErrorAdapter.OnItemClickListener mOnItemClickListener;//声明自定义的接口
    //定义方法并传给外面的使用者
    public void setOnItemClickListener(InventoryNuclearDiskListErrorAdapter.OnItemClickListener listener) {
    ...
    public void onClick(View v) {
        ...
        if (mOnItemClickListener != null) {
            switch (v.getId()){
                default:
                    mOnItemClickListener.onItemClick(v, InventoryNuclearDiskListErrorAdapter.ViewName.ITEM, position);
                    break;
            }
        }
    }
