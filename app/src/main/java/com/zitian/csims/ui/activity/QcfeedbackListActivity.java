...
import com.zitian.csims.ui.adapter.QcfeedbackListAdapter;
...
public class QcfeedbackListActivity extends BaseActivity implements  View.OnClickListener, HttpListener<String> {
    ...
    private void initView() {
        ...
        mDataAdapter = new QcfeedbackListAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    QcfeedbackList.Bean item = mDataAdapter.getDataList().get(position);
    ...
    private QcfeedbackListAdapter mDataAdapter = null;
    ...
    {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.QcfeedbackList(), RequestMethod.POST);
        ...
        SENDSMSRequest.add("type",CSIMSApplication.getAppContext().getUser().getZy_id());
    ...
    private void addItems(List<QcfeedbackList.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case 1000://快速登录
                QcfeedbackList qcfeedbackList = gson.fromJson(response.get(), QcfeedbackList.class);
                if (qcfeedbackList.getResult() == 1) {
                    if (qcfeedbackList.getData() != null) {
                        addItems(qcfeedbackList.getData());
                        TOTAL_COUNTER = qcfeedbackList.getTotal();
                        mRecyclerView.refreshComplete(REQUEST_COUNT);
                    } else {
                    }
                }else
                {
                    ToastUtil.show(this,qcfeedbackList.getMsg());
                }
                break;
     ...
     * item＋item里的控件点击监听事件
     */
    private QcfeedbackListAdapter.OnItemClickListener MyItemClickListener = new QcfeedbackListAdapter.OnItemClickListener() {

        @Override
        public void onItemClick(View v, QcfeedbackListAdapter.ViewName viewName, int position) {
            //viewName可区分item及item内部控件
            switch (v.getId()){
                case R.id.ItemDetail:
                    QcfeedbackList.Bean itemModel = mDataAdapter.getDataList().get(position);
                    Bundle mBundle = new Bundle();
                    mBundle.putSerializable("itemModel",itemModel);
                    openActivity(QcfeedbackDetailActivity.class,mBundle);
                    break;
            }
        }
