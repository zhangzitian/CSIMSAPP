...
import com.zitian.csims.ui.adapter.InventoryContrastManualAddItemListAdapter;
...
public class InventoryContrastManualAddItemListActivity extends BaseActivity implements  View.OnClickListener, HttpListener<String> {
    ...
    List<InventoryContrastManualAddItemList.Bean> list = new ArrayList<>();

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inventory_contrast_manual_add_item_list);
        mCurrentPage = 0;
        initView();
        setViewListener();
        DataBind3(true);
    ...
    private void initView() {
        ...
        mDataAdapter = new InventoryContrastManualAddItemListAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    InventoryContrastManualAddItemList.Bean item = mDataAdapter.getDataList().get(position);
    ...
    private void DataBind2(boolean isLoading) {
        Gson gson = new Gson();
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DoInventoryContrastDataAndManualScatteredAddItemEdit(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        ...
        SENDSMSRequest.add("addItemJson", list.size() == 0 ? "" : gson.toJson(list) );
    ...
    private void DataBind3(boolean isLoading) {
        Gson gson = new Gson();
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DoInventoryContrastDataAndManualScatteredAddItemList(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("sEcho", "1");
        SENDSMSRequest.add("UserId", CSIMSApplication.getAppContext().getUser().getO_id() );
        SENDSMSRequest.add("UserName", CSIMSApplication.getAppContext().getUser().getO_name());
    ...
    private InventoryContrastManualAddItemListAdapter mDataAdapter = null;
    ...
    private void addItems(List<InventoryContrastManualAddItemList.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            ...
            case 1001://快速登录
                ...
                break;
            case 1002://快速登录
                InventoryContrastManualAddItemList inventoryContrastManualAddItemList = gson.fromJson(response.get(), InventoryContrastManualAddItemList.class);
                if (inventoryContrastManualAddItemList.getResult() == 1) {
                    if(inventoryContrastManualAddItemList.getData()!=null)
                    {
                        list = inventoryContrastManualAddItemList.getData();
                        DataBind();
                    }
                }else
                {
                    ToastUtil.show(this, inventoryContrastManualAddItemList.getMsg());
                }
                break;
        }
    }
     ...
     * item＋item里的控件点击监听事件
     */
    private InventoryContrastManualAddItemListAdapter.OnItemClickListener MyItemClickListener = new InventoryContrastManualAddItemListAdapter.OnItemClickListener() {
        @Override
        public void onItemClick(View v, InventoryContrastManualAddItemListAdapter.ViewName viewName, int position) {
            ...
            switch (v.getId()) {
                case R.id.btnGain:
                    ...
                    InventoryContrastManualAddItemList.Bean  itemModel = mDataAdapter.getDataList().get(position);
                    //itemModel.id = 100;
    ...
    {
        builder = new CustomDialogStyle3.Builder(InventoryContrastManualAddItemListActivity.this);
        builder.setTitle("库位号:"+wareHouseNo)
                ...
                .setPositionButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        {
                            Toast.makeText(InventoryContrastManualAddItemListActivity.this,"产品编码不能为空！",Toast.LENGTH_SHORT).show();
                            return;
                        }

                        InventoryContrastManualAddItemList.Bean bean = new InventoryContrastManualAddItemList.Bean();
    ...
    public void UpdateCustomDialogStyle(InventoryContrastManualAddItemList.Bean bean,int pos)
    {
        builder = new CustomDialogStyle3.Builder(InventoryContrastManualAddItemListActivity.this);
        builder.setTitle("库位号:"+wareHouseNo)
                //.setMessage("请输入产品编码")
