...
import com.zitian.csims.ui.adapter.DailyErrorForkliftWarehousingTaskListAdapter;
...
public class DailyErrorForkliftWarehousingTaskListActivity  extends BaseActivity implements  View.OnClickListener, HttpListener<String> {

    private ImageView icon_back;
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_daily_error_forklift_warehousing_task_list);
    ...
    private void initView() {
        ...
        mDataAdapter = new DailyErrorForkliftWarehousingTaskListAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    DailyErrorForkliftWarehousingTaskList.Bean item = mDataAdapter.getDataList().get(position);
    ...
    private DailyErrorForkliftWarehousingTaskListAdapter mDataAdapter = null;
    ...
    {
        String condition =  "[t_taskType] = '盘点入库任务'  ";
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DailyErrorForkliftWarehousingTaskList(), RequestMethod.POST);
    ...
    private void addItems(List<DailyErrorForkliftWarehousingTaskList.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case 1000://快速登录
                DailyErrorForkliftWarehousingTaskList outofSpaceForkliftList = gson.fromJson(response.get(), DailyErrorForkliftWarehousingTaskList.class);
     ...
     * item＋item里的控件点击监听事件
     */
    private DailyErrorForkliftWarehousingTaskListAdapter.OnItemClickListener MyItemClickListener = new DailyErrorForkliftWarehousingTaskListAdapter.OnItemClickListener() {

        @Override
        public void onItemClick(View v, DailyErrorForkliftWarehousingTaskListAdapter.ViewName viewName, int position) {
            //viewName可区分item及item内部控件
            switch (v.getId()){
                case R.id.tv_fromNoAndArea:
                    DailyErrorForkliftWarehousingTaskList.Bean itemModel = mDataAdapter.getDataList().get(position);
