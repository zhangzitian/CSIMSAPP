...
import com.zitian.csims.ui.adapter.DailyErrorManagerTaskDetailsAdapter;
import com.zitian.csims.ui.adapter.DailyErrorManagerTaskListAdapter;
import com.zitian.csims.ui.adapter.ListBaseAdapter;
import com.zitian.csims.ui.adapter.SuperViewHolder;
...
public class DailyErrorManagerTaskListActivity extends BaseActivity implements  View.OnClickListener, HttpListener<String> {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_daily_error_manager_task_list);
    ...
    private void initView() {
        ...
        tvDate.setText(t1 + "盘点列表" );

        mRecyclerView = (LRecyclerView) findViewById(R.id.list);
        mDataAdapter = new DailyErrorManagerTaskListAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    //DailyErrorManagerTaskList.Bean item = mDataAdapter.getDataList().get(position);
    ...
    private DailyErrorManagerTaskListAdapter mDataAdapter = null;
    ...
    {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DailyErrorManagerTaskList(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("sEcho", "1");
        SENDSMSRequest.add("condition", "d_isPrint='false'");
    ...
    private void DataBind01(boolean isLoading)
    {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DailyErrorManagerTaskDetails2(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("sEcho", "1");
        CallServer.getInstance().add(this, 1001, SENDSMSRequest, this, false, isLoading);
    }

    private void DataBind02(boolean isLoading)
    {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DailyErrorManagerTaskDetails3(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("sEcho", "1");
        CallServer.getInstance().add(this, 1001, SENDSMSRequest, this, false, isLoading);
    }

    private void DataBind03(boolean isLoading)
    {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DailyErrorManagerTaskDetails4(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("sEcho", "1");
        SENDSMSRequest.add("iDisplayStart",mCurrentPage * REQUEST_COUNT);
        SENDSMSRequest.add("iDisplayLength", REQUEST_COUNT);
    ...
    private void DataBind04(boolean isLoading)
    {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.DailyErrorManagerTaskDetails5(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("sEcho", "1");
        CallServer.getInstance().add(this, 1001, SENDSMSRequest, this, false, isLoading);
    }
    ...
    private void addItems(List<DailyErrorManagerTaskList.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case 1000://快速登录
                DailyErrorManagerTaskList outofSpaceForkliftList = gson.fromJson(response.get(), DailyErrorManagerTaskList.class);
                ...
                break;
            case 1001://快速登录
                outofSpaceForkliftList = gson.fromJson(response.get(), DailyErrorManagerTaskList.class);
                if (outofSpaceForkliftList.getResult() == 1) {
     ...
     * item＋item里的控件点击监听事件
     */
    private DailyErrorManagerTaskListAdapter.OnItemClickListener MyItemClickListener = new DailyErrorManagerTaskListAdapter.OnItemClickListener() {
        @Override
        public void onItemClick(View v, DailyErrorManagerTaskListAdapter.ViewName viewName, int position) {
            //viewName可区分item及item内部控件
            switch (v.getId()){
                case R.id.tv_fromNoAndArea:
                    DailyErrorManagerTaskList.Bean itemModel = mDataAdapter.getDataList().get(position);
                ...
                case R.id.btnOperation:
                    ...
                    mBundle.putSerializable("itemModel",itemModel);
                    mBundle.putInt("mType",1);
                    openActivity(DailyErrorManagerTaskDetailsActivity.class,mBundle);
                    //itemModel.id = 100;
