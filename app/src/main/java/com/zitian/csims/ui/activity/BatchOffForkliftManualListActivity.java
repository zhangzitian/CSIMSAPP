...
import com.zitian.csims.model.BatchOffForkliftManualList;
...
import com.zitian.csims.ui.adapter.BatchOffForkliftManualListAdapter;
...
public class BatchOffForkliftManualListActivity   extends BaseActivity implements  View.OnClickListener, HttpListener<String> {

    private ImageView icon_back;
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_batch_off_forklift_manual_list);
    ...
    private void initView() {
        ...
        mDataAdapter = new BatchOffForkliftManualListAdapter(this);
        ...
        mLRecyclerViewAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (mDataAdapter.getDataList().size() > position) {
                    BatchOffForkliftManualList.Bean item = mDataAdapter.getDataList().get(position);
    ...
    private BatchOffForkliftManualListAdapter mDataAdapter = null;
    ...
    {
        String condition =  "[t_taskType] = '批量上架'  ";
        //condition += " and (t_taskState =  '未领取' or t_taskState =  '已领取')  and t_receiver = '"+"李什么"+"'";
        condition += " and (t_taskState =  '未领取' or t_taskState =  '已领取')  and t_receiver = '"+ CSIMSApplication.getAppContext().getUser().getO_id() +"'";

        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.BatchOffForkliftManualList(), RequestMethod.POST);
...
//        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.BatchOffForkliftManualList2, RequestMethod.POST);
//        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
//        SENDSMSRequest.add("userName", "userName");
//        SENDSMSRequest.add("t_ID", id);
//        //SENDSMSRequest.add("state", "");
//        CallServer.getInstance().add(this, 1000, SENDSMSRequest, this, false, isLoading);
//    }

    private void DataBind3(boolean isLoading) {
        Request<String> SENDSMSRequest = NoHttp.createStringRequest(AppConstant.BatchOffForkliftManualList3(), RequestMethod.POST);
        SENDSMSRequest.addHeader("token", "79FAF822-7194-4FE3-8C4F-1D99BE71BC9C");
        SENDSMSRequest.add("t_ID", itemModel.getT_ID());
        SENDSMSRequest.add("wh_wareHouseNo", itemModel.getT_toNo());//库位号
        SENDSMSRequest.add("t_prodNo",itemModel.getT_prodNo());//产品编码
        SENDSMSRequest.add("t_prodName",itemModel.getT_prodName());//产品名称
        SENDSMSRequest.add("t_count", itemModel.getT_count());//产品数量
        SENDSMSRequest.add("t_nbox_num", itemModel.getT_nbox_num());//产品件数
        SENDSMSRequest.add("t_npack_num",itemModel.getT_npack_num());//产品包数
        SENDSMSRequest.add("t_nbook_num", itemModel.getT_nbook_num());//产品册数
        SENDSMSRequest.add("t_exception2",itemModel.getT_exception2());//产品系列号
        SENDSMSRequest.add("t_exception", itemModel.getT_exception());//质量暂存区库位ID
        SENDSMSRequest.add("t_exception3", itemModel.getT_exception3());//质量暂存区库位ID
    ...
    private void addItems(List<BatchOffForkliftManualList.Bean> list) {
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case 1000://快速登录
                BatchOffForkliftManualList outofSpaceForkliftList = gson.fromJson(response.get(), BatchOffForkliftManualList.class);
                ...
                break;
            case 1001:
                outofSpaceForkliftList = gson.fromJson(response.get(), BatchOffForkliftManualList.class);
                if (outofSpaceForkliftList.getResult() == 1) {
                    //addItems(outofSpaceForkliftList.getData());
                    //TOTAL_COUNTER = outofSpaceForkliftList.getTotal();
                    //mRecyclerView.refreshComplete(REQUEST_COUNT);
    ...
    BatchOffForkliftManualList.Bean itemModel = null;
    int pos = 0;
    /**
     * item＋item里的控件点击监听事件
     */
    private BatchOffForkliftManualListAdapter.OnItemClickListener MyItemClickListener = new BatchOffForkliftManualListAdapter.OnItemClickListener() {

        @Override
        public void onItemClick(View v, BatchOffForkliftManualListAdapter.ViewName viewName, int position) {
            ...
            switch (v.getId()){
                case R.id.tv_fromNoAndArea:
                    ...
                    mBundle.putString("sourceType","质量上架纠错");
